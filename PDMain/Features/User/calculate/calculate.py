"""Calculate cog suit point progression for Paws Pendragon TTR.

This module provides the /calculate command for determining suit point progression,
activity recommendations, and laff boost milestone detection.
...
-------
    ...
    build_faction_thread_embeds()   Generates faction-specific embeds for threads.
...
--------
  ...
  Faction thread      -- build_faction_thread_embeds()
  UI components       -- Modals, selects, and views for dropdown flow
...
class Activity:
    """Represents a cog suit activity with point rewards."""
    ...
    def avg_pts(self) -> int:
        """Average points per activity run."""
        return (self.min_pts + self.max_pts) // 2

    @property
    def range_str(self) -> str:
        """Formatted point range (e.g. '1,525–1,630')."""
...
def _norm(s: str) -> str:
    """Normalize string for case-insensitive matching (alphanumeric only)."""
...
def resolve_suit(raw: str) -> tuple[str, str, str, str, bool] | None:
    """Resolve suit input to (user_abbr, display_name, faction, chart_key, is_v2).

    Accepts full names (Sellbot, sellbot), abbreviations (RB, rb),
    and 2.0 suffixes (RB2.0, RB20, RBv2).

    Returns None if suit not found.
    """
    is_v2 = False
    s = raw.strip()
...
def get_quota(user_abbr: str, faction: str, chart_key: str, level: int, is_v2: bool) -> int | None:
    """Get point quota for a suit at a given level.

    Args:
        user_abbr: Two-letter suit abbreviation (e.g., 'RB')
        faction: Faction key (e.g., 'cashbot')
        chart_key: Chart key for quota lookup (e.g., 'RB')
        level: Suit level (1-50)
        is_v2: Whether to use 2.0 quotas

    Returns:
        Point quota for the level, or None if not found.
    """
    if is_v2:
...
def valid_level_range(user_abbr: str, faction: str, chart_key: str, is_v2: bool) -> tuple[int, int]:
    """Get valid level range for a suit."""
    if is_v2:
...
def _ceil_runs(pts: int, act: Activity) -> int:
    """Calculate runs needed, ceiling division."""
    return max(1, math.ceil(pts / act.avg_pts)) if pts > 0 else 0


def _plan_lines(plan: list[tuple[Activity, int]]) -> str:
    """Format activity plan as markdown lines."""
    return "\n".join(
        f"• **{r}×** {a.name}  *({a.range_str} pts/run)*" for a, r in plan if r > 0
    )
...
def build_result_embed(
    ...
    """Build the main calculator result embed."""
    ...
    for opt in options:
        runs = opt["total"]
        embed.add_field(
            name=(f"{opt['emoji']} {opt['label']}" f" — {runs} run{'s' if runs != 1 else ''}"),
...
def build_faction_thread_embeds(faction_key: str) -> list[discord.Embed]:
    ...
    for abbr, name in suits:
        ...
        for lv in sorted(normal_levels):
            ...
            if pts == 0:
                ...
                lines.append(f"Lvl {lv} — {pts:,} {currency} — {_runs(pts)}× {best_short}")
    ...
    for lv in sorted(disguise_lvls):
        ...
        if pts == 0:
            ...
            rows2.append(f"Lvl {lv} — {pts:,} {currency} — {_runs(pts)}× {best_short}")
    ...
    for lv in sorted(top_data_v2):
        ...
        if pts == 0:
            ...
            rows3.append(f"Lvl {lv} — {pts:,} {currency} — {n}× {best_short}")
...
class _CalcView(discord.ui.View):
    """Main view container for calculator UI flow."""
...
class _SuitSelect(discord.ui.Select):
    """Second dropdown: choose suit within faction."""
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if abbr in _V2_SUITS:
            ...
            await interaction.response.send_modal(
                _SuitModal(abbr, suit_name, self.faction, False)
            )


class _VersionSelect(discord.ui.Select):
    """Third dropdown (for top-tier suits only): 1.0 or 2.0."""
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_modal(
            _SuitModal(self.abbr, self.suit_name, self.faction, is_v2)
        )


class _RestartView(discord.ui.View):
    """View with button to restart the calculator flow."""
...
class _SuitModal(discord.ui.Modal):
    """Combined modal: enter level and current points, then show result."""
    ...
    def __init__(self, abbr: str, suit_name: str, faction: str, is_v2: bool) -> None:
        v2_tag = " 2.0" if is_v2 else ""
        super().__init__(title=f"{suit_name}{v2_tag}")
        self.abbr = abbr
        self.suit_name = suit_name
        self.faction = faction
        self.is_v2 = is_v2
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        try:
            ...
            await interaction.response.edit_message(
                content="❌ Please enter a valid level number (e.g. 12).", view=None
            )
            return
        ...
        if not (lo <= level_num <= hi):
            ...
            await interaction.response.edit_message(
                content=f"❌ Invalid level. **{self.suit_name}{v2_tag}** levels range from {lo} to {hi}.",
                view=None,
            )
            return
        ...
        if quota is None:
            await interaction.response.edit_message(
                content=f"❌ Level {level_num} is not available.", view=None
            )
            return
        ...
        embed = build_result_embed(
            self.suit_name, self.faction, level_num, current_pts, quota, self.is_v2, options
        )
...
async def _reject_if_banned(interaction: discord.Interaction) -> bool:
    """Check ban status and send ephemeral rejection if banned.
    ...
    log.info(
        "Blocked banned user %s (id=%s) from calculate",
        interaction.user,
        interaction.user.id,
    )
    return True
...
def register_calculate(bot: Bot) -> None:
    """Register the /calculate command with the bot.
    ...
    async def calculate(interaction: discord.Interaction) -> None:
        """Calculate suit progression and activity recommendations.

        This is the /calculate slash command entry point. It starts the
        interactive dropdown flow to gather suit, level, and points info.
        """
        if await _reject_if_banned(interaction):
            return
    ...
    log.info("Registered /calculate command")
