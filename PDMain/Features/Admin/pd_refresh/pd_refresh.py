...
from Features.Core.config.constants import PD_REFRESH_COOLDOWN_SECONDS
...
_REFRESH_COOLDOWN = PD_REFRESH_COOLDOWN_SECONDS


class ErrorMessages:
    """Consistent user-facing message templates for slash command responses."""
    TEMPORARY = "⏱️ {action} is slow right now. Please try again in a moment."
    TIMEOUT = "⏱️ Operation timed out. Please try again."
    RATE_LIMIT = "🚫 You can use `/{cmd}` again in **{wait}**."
    PERMISSION = "🔐 You need **{perm}** permission to run this command."
    NOT_FOUND = "❌ {entity} not found. Please run `/pd-setup` first."
    SUCCESS = "✅ {action} complete."
    PARTIAL = "⚠️ {action} completed with errors — check console logs."
...
def register_pd_refresh(bot: TTRBot) -> None:
    ...
    async def pd_refresh(interaction: discord.Interaction) -> None:
        ...
        if not can_bypass:
            ...
            last_used: float = bot._refresh_cooldowns.get(cooldown_key, 0.0)
            remaining: float = _REFRESH_COOLDOWN - (time.time() - last_used)
            if remaining > 0:
                mins, secs = divmod(int(remaining), 60)
                wait: str = f"{mins}m {secs}s" if mins else f"{secs}s"
                await interaction.response.send_message(
                    ErrorMessages.RATE_LIMIT.format(cmd="pd-refresh", wait=wait),
                    ephemeral=True,
                )
                return
        ...
        if not can_bypass:
            bot._refresh_cooldowns[cooldown_key] = time.time()
        ...
        if refreshed_count:
            response_parts.append(
                ErrorMessages.SUCCESS.format(action="Feed refresh")
            )
        elif error_occurred:
            response_parts.append(ErrorMessages.PARTIAL.format(action="Refresh"))
        else:
            response_parts.append(ErrorMessages.TEMPORARY.format(action="TTR API"))
        ...
        if error_occurred and refreshed_count:
            response_parts.append(
                ErrorMessages.PARTIAL.format(action="Some operations")
            )
