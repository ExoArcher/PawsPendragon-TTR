...
class MessageSweep:
    ...
    def __init__(self, bot: TTRBot) -> None:
        ...
        """
        self.bot = bot
        # Allow override via SWEEP_INTERVAL env var (in seconds); default 15 minutes
        sweep_seconds = int(self.bot.config.sweep_interval) if hasattr(self.bot.config, 'sweep_interval') else 15 * 60
        self._sweep_loop.change_interval(seconds=sweep_seconds)
    ...
    async def _sweep_once(self) -> None:
        ...
        for guild_id_str in list(guilds_block.keys()):
            ...
            # Skip if guild is not in the allowlist or doesn't exist
            ...
            try:
                ...
                if swept:
                    log.info(
                        "Sweep: removed %d stale message(s) in guild %d",
                        swept, guild_id
                    )
            except Exception:
                log.exception("Sweep failed for guild %d", guild_id)
    ...
    async def _sweep_guild(self, guild_id: int) -> int:
        ...
        for entry in guild_state.values():
            ...
            if not isinstance(channel, discord.TextChannel):
                log.debug(
                    "Channel %d not found or not a text channel in guild %d",
                    channel_id, guild_id
                )
                continue
